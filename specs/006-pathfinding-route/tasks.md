...
- [ ] T001 Add pathfinding-specific verification notes or scripts in `package.json`
- [ ] T002 Ensure the active feature documentation set is present and aligned in `specs/006-pathfinding-route/`
- [ ] T003 [P] Update generated-artifact ignore coverage if needed for route-preview acceptance outputs in `.gitignore`
...
- [ ] T004 Create route-preview, route-step, and route-progress definitions in `src/engine/scenario/types.ts`
- [ ] T005 [P] Implement reusable preview-state helpers for ownership, replacement, and completion in `src/engine/map/routePreviewState.ts`
- [ ] T006 [P] Implement weighted pathfinding and neighbor expansion using current movement legality rules in `src/engine/map/routePathfinding.ts`
- [ ] T007 [P] Extend route and movement helpers for multi-step traversal and continuation seams in `src/engine/map/heroActions.ts`
- [ ] T008 [P] Extend game state initialization and turn-state preservation for route previews in `src/app/state/gameState.ts`
- [ ] T009 Create foundational integration coverage for route-preview state and weighted route resolution in `tests/integration/foundation/routePreviewStateFlow.test.ts`
...
- [ ] T010 [P] [US1] Add contract coverage for route-preview ownership and replacement state in `tests/contract/route-preview-state.contract.test.ts`
- [ ] T011 [P] [US1] Add integration coverage for reachable and unreachable route plotting in `tests/integration/map/routePreviewFlow.test.ts`
- [ ] T012 [P] [US1] Add browser acceptance coverage for first-click route preview in `tests/acceptance/route-preview.spec.ts`

### Implementation for User Story 1

- [ ] T013 [P] [US1] Extend map click handling to plot or replace preview routes without moving the hero in `src/app/scene-controller/mapInputController.ts`
- [ ] T014 [P] [US1] Implement route-preview feedback and invalid-destination reporting in `src/engine/map/terrainFeedback.ts`
- [ ] T015 [P] [US1] Render dotted route previews and destination markers in `src/render/canvas/renderMapScene.ts`
- [ ] T016 [US1] Surface active route preview details in `src/ui/hud/mapHud.ts`
- [ ] T017 [US1] Wire preview-specific sidebar messaging into `src/app/scene-controller/mapScene.ts`
...
- [ ] T018 [P] [US2] Add contract coverage for confirmation and partial traversal outcomes in `tests/contract/route-confirmation.contract.test.ts`
- [ ] T019 [P] [US2] Add integration coverage for confirmed route execution and partial movement in `tests/integration/map/routeCommitFlow.test.ts`
- [ ] T020 [P] [US2] Add browser acceptance coverage for second-click route confirmation in `tests/acceptance/route-confirmation.spec.ts`

### Implementation for User Story 2

- [ ] T021 [P] [US2] Implement route confirmation and partial traversal execution in `src/engine/map/heroActions.ts`
- [ ] T022 [P] [US2] Extend input handling to distinguish preview clicks from confirmation clicks in `src/app/scene-controller/mapInputController.ts`
- [ ] T023 [P] [US2] Extend route pathfinding and route rules to expose step-by-step movement cost sequences in `src/engine/map/routePathfinding.ts`
- [ ] T024 [US2] Surface partial traversal outcomes and remaining-route messaging in `src/ui/overlays/guardStatusOverlay.ts`
- [ ] T025 [US2] Integrate post-movement route completion or retention into map scene updates in `src/app/scene-controller/mapScene.ts`
...
- [ ] T026 [P] [US3] Add contract coverage for route persistence, revalidation, and ownership across turns in `tests/contract/route-persistence.contract.test.ts`
- [ ] T027 [P] [US3] Add integration coverage for cross-turn continuation and replacement in `tests/integration/map/routePersistenceFlow.test.ts`
- [ ] T028 [P] [US3] Add browser acceptance coverage for multi-turn route continuation in `tests/acceptance/route-persistence.spec.ts`

### Implementation for User Story 3

- [ ] T029 [P] [US3] Preserve and revalidate unfinished route previews across turn changes in `src/app/state/gameState.ts`
- [ ] T030 [P] [US3] Extend end-turn and hero-selection behavior to retain, invalidate, or replace route ownership correctly in `src/engine/turn/turnEngine.ts`
- [ ] T031 [P] [US3] Keep persistent route rendering and destination markers visible after turn changes in `src/render/canvas/renderMapScene.ts`
- [ ] T032 [US3] Extend HUD messaging for continuation-ready routes and invalidated persisted routes in `src/ui/hud/mapHud.ts`
- [ ] T033 [US3] Integrate cross-turn continuation and replacement flow in `src/app/scene-controller/mapScene.ts`
...
- [ ] T034 [P] Add any shared route-preview visual constants or sprite helpers in `src/render/sprites/placeholders.ts`
- [ ] T035 [P] Update route-preview run instructions and scenario notes in `specs/006-pathfinding-route/quickstart.md`
- [ ] T036 Run the full route-preview verification flow and record results in `specs/006-pathfinding-route/tasks.md`
- [ ] T037 Verify `spec.md`, `plan.md`, `data-model.md`, `contracts/`, and `tasks.md` remain consistent after implementation in `specs/006-pathfinding-route/tasks.md`
