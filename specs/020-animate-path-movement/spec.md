...
**Input**: User description: "When traversing the map, the hero instantly teleports to the destination. Change this to animate movement along the path at one tile per second, make the behavior configurable in settings, and move game template switching into a dedicated persistent settings page."
...
1. **Given** a hero has a confirmed route containing several legal steps and animated movement is enabled, **When** movement begins, **Then** the hero advances through the route one tile at a time in the displayed order at one completed tile per second.
...
As a player, I can choose whether route movement is animated or immediate from Settings, so that I can use the movement presentation that suits me.

**Why this priority**: A player-controlled option preserves a fast play style while making animation the default experience.

**Independent Test**: Open Settings, change the movement behavior, start routes in both modes, and verify that the selected option produces either one-tile-per-second traversal or immediate completion without changing route legality or outcomes.

**Acceptance Scenarios**:

1. **Given** the game opens without an existing setting, **When** the player starts a scenario and confirms a route, **Then** the hero uses animated movement at one tile per second.
...
3. **Given** the Settings page is open, **When** the player changes movement behavior back to animated, **Then** the next confirmed route again advances one tile per second.
...
- **FR-001**: The system MUST animate a hero's confirmed map route by advancing through its legal route steps in order when animated movement is enabled.
...
- **FR-008**: The Settings page MUST display and allow selection of animated or immediate map route movement.
- **FR-009**: Animated movement at one tile per second MUST be the default movement behavior when no prior selection exists.
- **FR-010**: When immediate movement is selected, confirmed routes MUST retain the current immediate-completion presentation while following the same legal endpoint and movement outcome as animated movement.
...
- **Movement Behavior**: The player's selected presentation for confirmed map routes: animated at one tile per second or immediate.
...
- **SC-003**: In settings testing, 100% of sampled changes between animated and immediate movement affect the next confirmed route as selected.
...
- Immediate movement remains available as a configurable alternative and retains its existing visible behavior.
