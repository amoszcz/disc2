...
3. **Given** two candidate routes differ only by movement-affecting static objects, **When** the player chooses between them, **Then** the actual movement spent reflects the object modifiers on the chosen tiles.
...
- What happens when a bridge object is placed on a non-river tile? The scenario must reject or ignore that invalid placement rather than silently changing unrelated terrain.
...
- What happens when two static movement objects could affect the same tile? The system must use a deterministic conflict rule rather than stacking ambiguously.
...
- **FR-011**: The system MUST use deterministic precedence when both terrain rules and static movement-object rules affect the same tile.
- **FR-012**: The system MUST reject or otherwise prevent invalid bridge placement on non-river tiles in scenario content.
- **FR-013**: The system MUST allow scenario definitions to place supported movement objects across the map as static authored content.
- **FR-014**: The system MUST keep hero movement validation consistent with both terrain and movement-object rules.
- **FR-015**: The system MUST preserve current blocked-terrain behavior for water tiles that are not explicitly bridged.
- **FR-016**: The system MUST communicate the movement effect of visible static movement objects to the player.
- **FR-017**: The system MUST visually distinguish bridges, milestones, and rubble from base terrain.
- **FR-018**: The system MUST explain when a move became legal because of a bridge object or changed cost because of a movement modifier object.
- **FR-019**: The system MUST keep movement-object behavior consistent across repeated turns and repeated route attempts in the same scenario.
- **FR-020**: The system MUST limit the first slice to scenario-authored static objects rather than player-built or destructible objects.
...
- **Movement Object Placement**: The authored scenario entry that declares which supported movement object occupies which map tile.
...
- **SC-002**: In scenario-content validation, 100% of invalid sampled bridge placements on non-river tiles are rejected or surfaced as invalid content.
...
- **SC-005**: In usability testing, at least 90% of players can correctly predict whether a bridge, milestone, or rubble object will make a route cheaper, more expensive, or newly traversable.
- **SC-006**: In repeated-turn testing, movement-object behavior remains consistent across 100% of sampled repeated route attempts in the same scenario.
...
- Milestones and rubble affect only the tile they occupy in the first slice.
...
- Arbitrary stacking of multiple movement-modifier objects on one tile beyond a deterministic first-slice conflict rule.
